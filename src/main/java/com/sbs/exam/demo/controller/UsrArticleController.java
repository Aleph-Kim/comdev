...
public class UsrArticleController {
	...
	public ResultData<List<Article>> getArticles() {
		return ResultData.from("S-12", "게시물 목록입니다.", articleService.getArticles());
	}
	...
	public ResultData<Article> getArticle(int id) {
		...
		return ResultData.from("S-11", Ut.f("%d번 게시물입니다.", id), article);
	}
	...
	public ResultData<Integer> doDelete(HttpSession httpSession, int id) {
		...
		if (article == null) {
			return ResultData.from("F-3", Ut.f("%d번 게시물이 존재하지 않습니다.", id));
		}

		memberId = (int) httpSession.getAttribute("LoginedMemberId");

		if (article.getMemberId() != memberId) {
			return ResultData.from("F-10", "게시물 삭제 권한이 없습니다.");
		}

		articleService.deleteArticle(id);
		return ResultData.from("S-3", Ut.f("%d번 게시물을 삭제 했습니다.", id), id);
	...
	public ResultData<Article> doModify(HttpSession httpSession, int id, String title, String body) {
		...
		return ResultData.from("F-3", Ut.f("%d번 게시물을 수정했습니다.", id), article);
	}
	// 액션 메서드 끝
}
